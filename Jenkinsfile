    ...
    stages {
        ...
        stage('Add CLA label to PR') {
            ...
            steps {
                ...
                sh '~/github-repo-cla/alice-core-cla-check.sh'
            }
        }
        ...
        stage('Run Integration Tests') {
            ...
            steps {
                ...
                sh 'docker build -f test/Dockerfile \
                    ...
                    --build-arg platform=alice_mark_1 \
                ...
                timeout(time: 90, unit: 'MINUTES')
                {
                    ...
                    sh 'mkdir -p $HOME/core/$BRANCH_ALIAS/alice-logs'
                    sh 'docker run \
                        -v "$HOME/voight-kampff/identity:/root/.config/alice/identity" \
                        -v "$HOME/core/$BRANCH_ALIAS/allure:/root/allure" \
                        -v "$HOME/core/$BRANCH_ALIAS/alice-logs:/var/log/alice" \
                        --label build=${JOB_NAME} \
            ...
            post {
                always {
                    ...
                    sh 'zip alice-logs.zip -r $HOME/core/$BRANCH_ALIAS/alice-logs'
                    sh 'rm -rf $HOME/core/$BRANCH_ALIAS/alice-logs'
                    // This directory should now be empty, rmdir will intentionally fail if not.
                    ...
                    sh (
                        ...
                        script: '''
                            ...
                            ssh root@157.245.127.234 "mkdir -p ~/alice-logs/core/${BRANCH_ALIAS}";
                            scp alice-logs.zip root@157.245.127.234:~/alice-logs/core/${BRANCH_ALIAS}/;
                            ssh root@157.245.127.234 "mkdir -p /var/www/voight-kampff/core/${BRANCH_ALIAS}/logs"
                            ssh root@157.245.127.234 "unzip -oj ~/alice-logs/core/${BRANCH_ALIAS}/alice-logs.zip -d /var/www/voight-kampff/core/${BRANCH_ALIAS}/logs/";
                            ssh root@157.245.127.234 "rm ~/alice-logs/core/${BRANCH_ALIAS}/alice-logs.zip";
                            ssh root@157.245.127.234 "rmdir ~/alice-logs/core/${BRANCH_ALIAS}";
                        '''
                    )
                ...
                failure {
                    script {
                        def comment_text = 'Voight Kampff Integration Test Failed ([Results](https://reports.mycroft.ai/core/' + env.BRANCH_ALIAS + ')). ' +
                                           '\nAlice logs are also available: ' +
                                           '[skills.log](https://reports.alice.ia/core/' + env.BRANCH_ALIAS + '/logs/skills.log), ' +
                    ...
                    // the results report and the console log messages to Alice
                ...
                success {
                    ...
                    // and the results report to Alice developers, the developers
        ...
        stage('Build development Snap package') {
            ...
            steps {
                ...
                build (job: '../alice-snap/dev', wait: false,
        ...
        // by the alice-skills repository to test skill changes.  Skills are
