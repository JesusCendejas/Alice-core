...
function help() {
    echo "${script}:  Alice command/service launcher"
    ...
    echo "  unittest                 run alice-core unit tests (requires pytest)"
...
function name-to-script-path() {
    case ${1} in
        "bus")               _module="alice.messagebus.service" ;;
        "skills")            _module="alice.skills" ;;
        "audio")             _module="alice.audio" ;;
        "voice")             _module="alice.client.speech" ;;
        "cli")               _module="alice.client.text" ;;
        "audiotest")         _module="alice.util.audio_test" ;;
        "wakewordtest")      _module="test.wake_word" ;;
        "enclosure")         _module="alice.client.enclosure" ;;
...
function launch-background() {
    ...
    if [[ "${1}" == "bus" ]] ; then
        echo "CAUTION: The Alice bus is an open websocket with no built-in security"
    ...
    python3 -m ${_module} $_params >> /var/log/alice/${1}.log 2>&1 &
}

function launch-all() {
    echo "Starting all alice-core services"
    ...
    if [ ! -f .installed ] || ! md5sum -c &> /dev/null < .installed ; then
        ...
        if [ "$auto_update" == "true" ] ; then
            ...
            if command -v notify-send >/dev/null ; then
                ...
                notify-send "Alice Dependencies Outdated" "Run ./dev_setup.sh again"
            fi
            exit 1
        fi
    fi
...
case ${_opt} in
    ...
    "unittest")
        source-venv
        pytest test/unittests/ --cov=alice "$@"
        ;;
    "singleunittest")
        source-venv
        pytest "$@"
    ...
    "skillstest")
        ...
        ;;
    "vktest")
        source "$DIR/bin/alice-skill-testrunner" vktest "$@"
        ;;
    "audiotest")
        launch-process ${_opt}
        ;;
